...
// Adapted for J2ME/CLDC 1.1 (it5260 build): java.security isn't part of CLDC,
// so the SHA-1-based hash-key generator has been replaced with a fixed-seed
// java.util.Random (see getRandomHashVal() near the bottom of this file).
// @Override annotations were also removed for older preverify-tool safety.
...
public class Position {
    ...
    // Must be declared (and thus initialized) before the static block below,
    // since that block calls getRandomHashVal(), which reads this field.
    private static final Random hashRnd = new Random(0x5EED1234L);
    ...
    // rndNo is intentionally unused: the static block above always calls this
    // the same number of times in the same fixed order every time the class
    // loads, so drawing the next value from one seeded stream is exactly as
    // deterministic as hashing rndNo through SHA-1 was - it just produces a
    // different (equally valid) set of Zobrist constants.
