...
// Adapted for J2ME/CLDC 1.1 (it5260 build): CLDC has no java.util.List or
// generics, so the one place this file used List<Move> (filter(), just
// below) now takes a plain Vector instead - Vector.contains(Object) exists
// in CLDC 1.1 same as List's, so the method body itself needed no change.
// Note: the commented-out "Extra debug checks" block further down uses
// ArrayList/HashSet and a diamond operator - it's dead code (inside a
// block comment) in the original too, so it was left untouched rather than
// ported; nothing calls it.
...
public final class MoveGen {
    ...
    public final static class MoveList {
        ...
        public final void filter(Vector searchMoves) {
            int used = 0;
